/*************************************************************
 * We solve the equation tau_{20}(p)^2-p^19 = \kappa \cdot q^b
 * ***********************************************************/
	...
	for level in levels do
		...
		for i in [1..#NFs] do
			...
			if Valuation(level,q) eq 0 then
				D:=[b: b in [0..(M-1)] | IsDivisibleBy(b,19) eq true];
			else
				D:=[b: b in [0..(M-1)] | IsDivisibleBy(b,19) eq false];
			end if;
			D1:=[b : b in D | kappa*(Integers(4)!q)^b eq 3];
			D3:=[b : b in D | kappa*(Integers(4)!q)^b eq 1];
			for l in L do
				if l in [3,5,7,11,13] eq false then
					prs:={<s,t> : t,s in GF(l) | s*(t^2-s^19) ne 0};
				elif l eq 3 then
					prs:={<s,s+1> : s in GF(l)};
				elif l eq 5 then
					prs:={<s,s*(s+1)> : s in GF(l)};
				elif l eq 7 then
					prs:={<s,s^2*(s^3+1)> : s in GF(l)};
				elif l eq 11 then
					prs:={<s,s*(s^7+1)> : s in GF(l)};
				elif l eq 13 then
					prs:={<s,s*(s^5+1)> : s in GF(l)};
				end if;
				prs:={pr : pr in prs | pr[1]*(pr[2]^2-pr[1]^19) ne 0};
				Cl1:=[];
				Cl3:=[];
				for pr in prs do
					E3:=EllipticCurve([0,2*pr[2],0,pr[1]^19,0]);			// p = 3 mod 4
					E1:=EllipticCurve([0,2*pr[2],0,pr[2]^2-pr[1]^19,0]);	// p = 1 mod 4
					if IsDivisibleBy(Integers()!(Norm(Trace(E3)-Coefficient(f,l))),19) then
						Include(~Cl3,(pr[2]^2-pr[1]^19));
					end if;
					if IsDivisibleBy(Integers()!(Norm(Trace(E1)-Coefficient(f,l))),19) then
						Include(~Cl1,(pr[2]^2-pr[1]^19));
					end if;
				end for;
